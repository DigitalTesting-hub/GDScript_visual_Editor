...
func _setup_detection_area():
	...
	print("Player DetectionArea on layer 5")
...
func _sync_collision_shapes():
	if not is_multiplayer_authority():
		...
		if sync_stance != last_sync_stance:
			last_sync_stance = sync_stance
...
func _physics_process(delta):
	...
	if is_firing_held and current_weapon == WeaponType.GUN:
		_update_look_target()
	...
	if _body:
		...
		_body.animate(velocity, current_stance, firing_state, is_aiming, look_target if has_look_target else Vector3.ZERO)
...
func _switch_to_spectator_camera():
	...
	# Switch to first alive player's camera
